...
if __name__ == "__main__":
    ...
    adaptive_pseudo_augment_prob = hp.AdaptivePeudoAugment.initial_prob
    lambda_r = 0.
    lambda_rs = []
    lambda_fs = []
            ...
            with cuda.amp.autocast(enabled=hp.use_fp16):
                ...
                for imgs, fakes in zip(imgs_batches, fakes_batches):
                    ...
                    with torch.inference_mode():
                        if hp.batch_sizeD != num_mix_fakes:
                            ...
                            for _fakes_out in fakes_out:
                                _lambda_f = logit_sigmoid(_fakes_out[:hp.batch_sizeG]).sign().mean().item()
                                lambda_fs.append(_lambda_f)
                    ...
                    if scaler is not None:
                        ...
                        lossD.backward()
                    del lossD, reals_out, fakes_out, imgs, fakes
                    del projected_imgs, projected_fakes
                    if "_reals_out" in globals():
                        del _reals_out
                    if "_fakes_out" in globals():
                        del _fakes_out
                del fakes_batches
                ...
                if (global_step + 1) % hp.AdaptivePeudoAugment.iteration_per == 0 and \
                    ...
                    if len(lambda_rs) != 0 or len(lambda_fs) != 0:
                        lambda_r = sum(lambda_rs) / len(lambda_rs)
                        lambda_f = sum(lambda_fs) / len(lambda_fs)
